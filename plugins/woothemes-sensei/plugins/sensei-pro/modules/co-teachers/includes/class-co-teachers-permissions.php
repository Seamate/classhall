...
class Co_Teachers_Permissions {
	...
	public static function init() {
		...
		add_filter( 'sensei_learners_allowed_user_ids', [ $instance, 'allow_student_management_for_coteachers' ], 10, 2 );
		add_filter( 'sensei_teacher_course_ids', [ $instance, 'add_coteacher_course_ids' ], 10, 2 );
	}
	...
	/**
	 * Allow co-teachers to manage students in a given course.
	 *
	 ...
	 */
	public function allow_student_management_for_coteachers( array $user_ids, int $course_id ): array {
		$coteacher_ids = $this->co_teachers->get_course_coteachers_ids( $course_id );
	...
	/**
	 * Add the course IDs that a given user is a co-teacher for to an existing array of
	 * course IDs that the user is a teacher for.
	 *
	 * @param int[] $course_ids Course IDs that a given user is a teacher for.
	 * @param int   $user_id    User ID.
	 *
	 * @return int[] Filtered course IDs that a given user is either a teacher or a co-teacher for.
	 */
	public function add_coteacher_course_ids( $course_ids, $user_id ) {
		$coteacher_course_ids = $this->co_teachers->get_coteacher_courses_ids( $user_id );

		return array_unique( array_merge( $course_ids, $coteacher_course_ids ) );
	}
