...
class Block_Purchase_Course {
	...
	private function render_single_product_form() {
		...
		// Use custom text with price for single product.
		/* translators: %1$s is the action text (e.g., 'Buy'), %2$s is the price (e.g., '$5.00') */
		$button_format = __( '%1$s - %2$s', 'sensei-pro' );
		$button_text   = sprintf( $button_format, $this->get_custom_text(), $price );
	...
	private function render_multiple_products_form() {
		// Get the price for the first product.
		$first_product = $this->products[0];
		$price         = $first_product->get_price_html();

		// Use custom text with price for multiple products.
		/* translators: %1$s is the action text (e.g., 'Buy'), %2$s is the price (e.g., '$5.00') */
		$button_format = __( '%1$s - %2$s', 'sensei-pro' );
		$button_text   = sprintf( $button_format, $this->get_custom_text(), $price );
		$button        = $this->render_button( $button_text );
		...
		// If the block is in a Course List block, don't render the products.
		return '
	...
	/**
	 * Get the custom text for the button, defaulting to "Buy" if not set or "Take Course".
	 *
	 * @return string The custom text.
	 */
	private function get_custom_text() {
		// Extract button text from the HTML content.
		$custom_text = '';

		if ( ! empty( $this->button ) ) {
			preg_match( '/<button[^>]*class="[^"]*wp-block-button__link[^"]*"[^>]*>(.*?)<\/button>/s', $this->button, $matches );

			if ( isset( $matches[1] ) ) {
				$custom_text = trim( wp_strip_all_tags( html_entity_decode( $matches[1] ) ) );
			}
		}

		// If no text found or it's "Take Course", default to "Buy".
		if ( ! $custom_text || __( 'Take Course', 'sensei-pro' ) === $custom_text ) {
			$custom_text = esc_html__( 'Buy', 'sensei-pro' );
		}
	...
	private function render_button( $text ) {
		// Get the original action text (without price) for use in frontend.js.
		$original_text = $this->get_custom_text();

		$button = preg_replace(
			'|<button(.*)>.*</button>|i',
			'<button $1 data-original-text="' . esc_attr( $original_text ) . '">' . wp_kses_post( $text ) . '</button>',
			$this->button
		);
