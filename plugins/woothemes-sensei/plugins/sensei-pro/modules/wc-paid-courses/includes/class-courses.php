...
final class Courses {
	...
	private function add_products_total_sales_field( $products ) {
		...
		if ( $count_variations > 0 ) {
			$paths = [
				'\Automattic\WooCommerce\Admin\API\Reports\GenericQuery'     => 'variations',
				'\Automattic\WooCommerce\Admin\API\Reports\Variations\Query' => null,
			];

			foreach ( $paths as $class => $param ) {
				if ( class_exists( $class ) ) {
					$query_variations = $param ? new $class( $args, $param ) : new $class( $args );
					break;
				}
			}

			// Query the variations sales through WC Analytics.
			if ( isset( $query_variations ) ) {
