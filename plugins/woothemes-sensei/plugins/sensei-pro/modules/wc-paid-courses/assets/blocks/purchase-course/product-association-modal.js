...
const useProductsSelection = ( initialSelectedProductIds ) => {
	...
	}, [ initialSelectedProductIds, updateModalSelectedProducts ] );
...
const ProductAssociationModal = ( {
	...
	const { selectedProductIds, toggleProduct, submitSelection } =
		useProductsSelection( initialSelectedProductIds );

	const { products, isLoadingProducts, hasAssignableProducts, onSearch } =
		useSearch();

	// New product modal.
	const [ isNewProductModalActive, setNewProductModalActive ] =
		useState( false );
						...
						<tr key={ product.id }>
							...
							<td>
								{ product.linked_courses.length === 0
									...
									: sprintf(
											// translators: %d: number of linked courses.
											_n(
												'%d linked course',
												'%d linked courses',
