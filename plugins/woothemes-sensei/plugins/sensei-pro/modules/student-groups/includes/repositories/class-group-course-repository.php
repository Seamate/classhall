...
class Group_Course_Repository {
	...
	/**
	 * Get group courses by access start date range.
	 *
	 * @param DateTime $from_timestamp Start time range of the access date.
	 * @param DateTime $to_timestamp   End of range of the access date.
	 * @param int      $last_index     Last index for skipping.
	 * @param int      $batch_size     Limit of results.
	 *
	 * @return Group_Course[]
	 */
	public function get_group_courses_by_access_start_date_range( $from_timestamp, $to_timestamp, $last_index = -1, $batch_size = 1000 ): array {
		$query = $this->wpdb->prepare(
			'SELECT * FROM ' . $this->table_name . ' WHERE access_period_start >= %s AND access_period_start < %s AND id > %d ORDER BY id LIMIT %d',
