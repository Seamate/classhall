...
class Wizard {
	...
	/**
	 * Redirect to the setup wizard if the current screen is Sensei LMS.
	 *
	 * @param \WP_Screen $screen The current screen object.
	 *
	 * @since 1.24.3
	 */
	public function redirect_home_to_setup_wizard( $screen ): void {
		if ( 'sensei-pro' === $screen->id ) {
			$this->redirect_to_setup_wizard();
		}
	}
	...
	public function register_wizard_page() {
		if ( ! self::is_sensei_activated() && ! self::$added_menu_page ) {
			...
			// Hide the first item in the submenu to avoid showing the "Sensei LMS" link twice.
			add_action(
				'admin_enqueue_scripts',
				function () {
					$custom_css = '#toplevel_page_sensei-pro .wp-submenu li.wp-first-item { display: none; }';
					wp_add_inline_style( 'admin-menu', $custom_css );
				}
			);

			add_action( 'current_screen', [ $this, 'redirect_home_to_setup_wizard' ] );
