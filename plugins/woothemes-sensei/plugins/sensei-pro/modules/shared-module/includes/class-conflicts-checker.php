...
class Conflicts_Checker {
	 ...
	 *     @type callable $get_message Callable for getting the message to be shown to the user that explains the conflict.
	 *   }
	 * }
	 */
	 ...
	 *   @type callable $get_message  Callback for getting the message to display to the user.
	...
	public function conflicts_with( array $conflict ): bool {
		if ( self::is_plugin_active( $conflict['plugin_slug'] ) ) {
			...
			$get_message  = $conflict['get_message'];
			$message_type = isset( $conflict['message_type'] ) ? $conflict['message_type'] : 'error';

			add_action(
				'admin_notices',
				function () use ( $get_message, $message_type ) {
					self::add_conflict_notice( $get_message(), $message_type );
				}
			);
