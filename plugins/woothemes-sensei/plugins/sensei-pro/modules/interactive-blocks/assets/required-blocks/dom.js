...
function CompleteLessonTooltip() {
	...
	const requiredBlockIds = useMemo(
		() =>
			selectors.filterRequiredBlockIds(
				{ attributes },
	...
	const allBlocksAreComplete = useMemo(
		() =>
			selectors.areRequiredBlocksCompleted(
				{ attributes },
				requiredBlockIds
			),
		[ attributes, requiredBlockIds ]
	);

	const uncompleteBlocksCount = useMemo( () => {
	...
	return (
		<Tooltip
			...
			className={ classnames(
				...
				{
					'sensei-supports-required__complete-lesson-overlay--completed':
						allBlocksAreComplete,
				}
			) }
...
export function addVideoCompletedStatus( id, blockElement ) {
	...
	createRoot( completedStatusWrapper ).render(
		<Provider store={ blocksStore }>
